...
页面随包安装，但**不冻结在打包那一刻**。站点会发布 `app-bundle.json`——每个文件
一条 SHA-256，版本号是这些哈希的哈希；APK 里带着自己那份。

更新就是一次集合差：只下载哈希和包内不同的路径，其余继续从 APK 里读。改一次前端
通常只有几个 JS/CSS 块变动，也就是几十 KB，而不是十五兆的字体和图版。

- 只在**不计流量的网络**上检查：启动后 20 秒一次，之后每天一次
- 下载落在 `<版本>.part`，**逐个校验哈希**后才改名就位，半个目录永远读不到
- 指针文件最后写，切换是一次原子重命名
- 指针在 Activity 启动时**只读一次**：更新若在有人阅读时装好，下次打开才生效，不会
  在页面底下换掉文件
- 任何一步出错就整体放弃，App 继续跑原来那份

因为这个机制要比对哈希，**站点和 App 现在共用同一个构建产物**——源站地址改成运行时
判断（`location.origin` 是不是 App 的资源域），不再是构建期开关。两边字节一致，比对
才有意义。

改了前端只要 `git push`，Netlify 部署完，装了 App 的人下次打开就是新的；只有改
Kotlin（新权限、新原生能力）才需要重新出 APK。
