...
function out(c, me, postOwnerId) {
  return {
    ...
    canDelete: me ? (c.authorId === me.id || postOwnerId === me.id || me.role === "admin") : false
  }
}
...
export default async (req, context) => {
  ...
  if (req.method === "GET") {
    ...
    return json({ comments: rows.map((c) => out(c, me, p.authorId)), locked: !!p.commentsLocked, canModerate: p.authorId === me.id || me.role === "admin" })
  }
  ...
  if (req.method === "DELETE") {
    ...
    if (c.authorId !== me.id && p.authorId !== me.id && me.role !== "admin") return oops("无权删除此评论", 403)
